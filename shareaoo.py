...
\033[1;38m~ \033[1;31mYoutube : \033[0;93m{__WEB__}
\033[1;37m- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
{ndp_tool}\033[1;37mCopyright By: \033[1;34m{__ADMIN__}
{ndp_tool}\033[1;35mZalo: \033[1;34m{__ZALO__}
{ndp_tool}\033[1;36mFacebook: \033[1;31mFb.com/{__ZALO__}
...
clear()
banner()
file = input(ndp_tool+luc+'Nhập Ci Phai Token Cấy'+trang+': '+vang)
try:
    listtaikhoan = open(file+'.txt', 'r', encoding='utf-8').read().split('\n')
except:
    quit(ndp_tool+do+'File Không Tồn Tai Tề Đừng Bỏ .txt vô')
# NHẬP THÔNG SỐ RUN TOOL
clear()
banner()
for line in listtaikhoan:
    count+=1
print(ndp_tool+luc+'File Chứa Token Là'+trang+': ',count)
thanh()
linkpost = input(ndp_tool+luc+'Link Bài Viết Thằng Ngu Ni'+trang+': '+vang)
...
if 'success' in get_id_post:
    ...
    thanh()
    exit(ndp+do+'ĐỊT MẸ BÀI VIẾT SAI TỀ THẰNG NGU')
thanh()
print(ndp_tool+do+'['+vang+'SUCCESS'+do+']'+trang+': '+xnhac+'UID POST MI LÀ'+trang+': ',id_post)
thanh()
delay = int(input(ndp_tool+luc+'NHẬP THỜI GIAN CHẬM NHẤT CẤY'+trang+': '+vang))
thanh()
while True:
